#!/usr/bin/env bash
# Startup fallback: ensure 3LA-Corners is loaded and its settings applied.
...
if ! hyprctl plugin list | grep -q "3LA-Corners"; then
    hyprctl plugin load "$DIR/3LA-Corners.so"
fi
