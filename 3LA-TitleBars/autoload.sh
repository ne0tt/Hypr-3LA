#!/usr/bin/env bash
# Startup fallback: ensure 3LA-TitleBars is loaded and its settings applied.
...
if ! hyprctl plugin list | grep -q "3LA-TitleBars"; then
    hyprctl plugin load "$DIR/3LA-TitleBars.so"
fi
