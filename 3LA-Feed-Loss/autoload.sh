#!/usr/bin/env bash
# Startup fallback: ensure 3LA-Feed-Loss is loaded and its settings applied.
...
if ! hyprctl plugin list | grep -q "3LA-Feed-Loss"; then
    hyprctl plugin load "$DIR/3LA-Feed-Loss.so"
fi
